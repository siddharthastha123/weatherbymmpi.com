  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      for (var i = 0; i < 7; i++) {
        ...
        let iconUrl = 'https://openweathermap.org/img/w/' + result.list[i].weather[0].icon + '.png'; // Use HTTPS
        ...
        // Convert high temperature to Celsius
        let highTempCelsius = fahrenheitToCelsius(highTemp);
        let displayHighTempCelsius = `High ${highTempCelsius}&#176;C`;
        $("#high" + int).append(` (${displayHighTempCelsius})`);

        let lowTemp = kelvinToFahrenheit(result.list[i].temp.min);
        let displayLowTemp = `Low ${lowTemp}&#176;F`;
        $("#low" + int).html(displayLowTemp);

        // Convert low temperature to Celsius
        let lowTempCelsius = fahrenheitToCelsius(lowTemp);
        let displayLowTempCelsius = `Low ${lowTempCelsius}&#176;C`;
        $("#low" + int).append(` (${displayLowTempCelsius})`);
      }
    }
  });
}
